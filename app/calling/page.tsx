...
export default function Calling() {
  ...
  async function loadProspects() {
    try { const res = await fetch("/api/prospects", { cache: "no-store" }); const body = await res.json(); setProspects(Array.isArray(body) ? body : []); } catch (e) { console.error(e); }
  }
  ...
  useEffect(() => {
    ...
    (async () => { try { const res = await fetch(`/api/notes?prospect_id=${current.p.id}`, { cache: "no-store" }); if (res.ok) setNotes(await res.json()); } catch (e) { /* */ } })();
    setNoteText("");
  }, [current?.p.id]);
  ...
  async function act(stage: string, note?: string) {
    if (!current) return; setWorking(true); const id = current.p.id;
    try {
      ...
      setIdx((i) => i + 1);
    } catch (e) { console.error("action failed", e); } finally { setWorking(false); }
  }
    ...
    <main className="min-h-screen bg-gray-50 px-4 sm:px-6 py-6">
      <div className="max-w-2xl mx-auto">
        <header className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">Appointment Setter Mode</h1>
          <Link href="/" className="text-sm text-blue-700 hover:underline">← Dashboard</Link>
        </header>

        {!current && (
          <div className="text-center text-gray-500 py-20">{queue.length === 0 && prospects.length > 0 ? "🎉 Call list complete — every qualified lead has been worked." : "No qualified leads in the queue yet. Run a scrape from the dashboard."}</div>
        )}
        ...
        {current && (
          <div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4 shadow-sm">
              ...
              <div className="text-sm text-gray-500 mb-3">{current.p.city}{current.p.state ? `, ${current.p.state}` : ""} · {current.p.rating ?? "—"}★ ({current.p.review_count ?? 0}) · {hi ? STATUS_LABEL[hi.status] : ""} · DM {current.dm}%</div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-3">
              <button disabled={working} onClick={() => act("Contacted")} className="bg-gray-800 text-white rounded-lg px-3 py-2 text-sm disabled:opacity-50">Contacted</button>
              <button disabled={working} onClick={() => act("Contacted", "Left voicemail")} className="bg-gray-600 text-white rounded-lg px-3 py-2 text-sm disabled:opacity-50">Voicemail</button>
              <button disabled={working} onClick={() => act("Lost", "Not interested")} className="bg-red-600 text-white rounded-lg px-3 py-2 text-sm disabled:opacity-50">Not Interested</button>
              <button disabled={working} onClick={() => act("Follow Up")} className="bg-amber-500 text-white rounded-lg px-3 py-2 text-sm disabled:opacity-50">Follow Up</button>
              <button disabled={working} onClick={() => act("Discovery Call", "Booked discovery call")} className="bg-green-600 text-white rounded-lg px-3 py-2 text-sm disabled:opacity-50">Booked Call</button>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2"><input className="border border-gray-300 rounded-lg px-2 py-1 text-sm flex-1" value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Add a note" onKeyDown={(e) => { if (e.key === "Enter") addNote(); }} /><button onClick={addNote} className="bg-blue-700 text-white rounded-lg px-3 py-1 text-sm">Add</button></div>
